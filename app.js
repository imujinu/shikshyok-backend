...
const { connectKafka, getConsumer } = require("./kafka");
  ...
  await consumer.run({
    eachMessage: async ({ message }) => {
      ...
      try {
        ...
        await t.commit();
        console.log("DB 저장 성공:", orderData);
      } catch (err) {
        await t.rollback();
        console.error("DB 저장 실패, 롤백됨:", err);

        // 롤백 증적 기록
        try {
          await Order.create({ ...orderData, option: "롤백" });
          console.log("롤백 증적 기록 완료:", orderData);
        } catch (err2) {
          console.error("롤백 증적 기록 실패:", err2);
        }
      }
    },
  });
